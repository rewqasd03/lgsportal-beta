              ...
              <div className="space-y-4">
                            ...
                            <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                              ...
                              <td className="px-2 py-2 text-center text-green-600 font-medium">
                                {(() => {
                                  ...
                                  const totalCorrect = Object.values(nets as {[key: string]: number}).reduce((sum: number, net) => {
                ...
                <div className="bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-lg shadow p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white bg-opacity-20 p-3 rounded-lg">
                      <h4 className="text-xs font-medium opacity-90">Hedef Net Uzaklığı</h4>
                      <p className="text-xl font-bold">
                        {(() => {
                          const currentNet = netChartData.slice(-3).reduce((sum, d) => sum + (d.öğrenci || 0), 0) / Math.min(3, netChartData.length);
                          const targetNetTotal = Object.values(studentTargets).reduce((sum, target) => sum + target, 0);
                          const diff = currentNet - targetNetTotal;
                ...
                <div className="bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-lg shadow p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <div className="bg-white bg-opacity-20 p-3 rounded-lg">
                      <h4 className="text-xs font-medium opacity-90">Son Puan</h4>
                      <p className="text-xl font-bold">
                        {scoreChartData.length > 0 ? scoreChartData[scoreChartData.length - 1].öğrenci?.toFixed(0) || '0' : '0'}
                      </p>
                    </div>
                    <div className="bg-white bg-opacity-20 p-3 rounded-lg">
                    ...
                    <div className="bg-white bg-opacity-20 p-3 rounded-lg">
                      ...
                      <p className="text-xl font-bold">
                        {scoreChartData.length > 0 ? ((scoreChartData[scoreChartData.length - 1].öğrenci || 0) / studentScoreTarget * 100).toFixed(0) : '0'}%
                      </p>
                    </div>
                  </div>
                </div>
                        ...
                        <div key={subject.key} className="bg-gray-50 rounded-lg p-4">
                          ...
                          <ResponsiveContainer width="100%" height={300}>
                            <LineChart data={subjectData}>
                              ...
                              />
                              <YAxis domain={[0, subject.key === 'turkce' || subject.key === 'matematik' || subject.key === 'fen' ? 20 : 10]} />
                              <Tooltip 
                                formatter={(value, name) => [`${Number(value).toFixed(1)}`, name]}
                                labelFormatter={(label) => `Deneme ${label}`}
                              />
