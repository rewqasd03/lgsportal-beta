  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(async (cache) => {
        ...
        // Mevcut dosyaları teker teker cache'le
        const cachePromises = urlsToCache.map(async (url) => {
          try {
            ...
            console.log('SW: Failed to cache (network)', url, error.message);
          }
        });
        
        // Tüm cache işlemlerinin bitmesini bekle
        await Promise.allSettled(cachePromises);
        console.log('SW: Cache process completed');
      })
  );
  self.skipWaiting();
});
